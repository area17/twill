<?php

return [
    'auth' => [
        'email' => 'Email',
        'password' => 'Mot de passe',
        'forgot-password' => 'Mot de passe oublié',
        'login' => 'S\'identifier',
        'login-title' => 'S\'identifier',
    ],
    'dashboard' => [
        'search-placeholder' => 'Tout rechercher...',
        'empty-message' => 'Vous n\'avez aucune activité pour le moment.',
        'all-activity' => 'Toutes les activités',
        'my-activity' => 'Mon activité',
        'create-new' => 'Créer',
        'my-drafts' => 'Mes brouillons',
        'statitics' => 'Statistiques',
    ],
    'footer' => [
        'version' => 'Version',
    ],
    'form' => [
        'content' => 'Contenu',
        'editor' => 'Editeur',
    ],
    'listing' => [
        'filter' => [
            'all-items' => 'Tout',
            'mine' => 'Mine',
            'published' => 'Publié(e)s',
            'draft' => 'Brouillons',
            'trash' => 'Corbeille',
        ],
        'dropdown' => [
            'edit' => 'Éditer',
            'unpublish' => 'Dépublier',
            'publish' => 'Publier',
            'unfeature' => 'Ne plus mettre en avant',
            'feature' => 'Mettre en avant',
            'restore' => 'Restaurer',
            'destroy' => 'Détruire',
            'delete' => 'Supprimer',
            'duplicate' => 'Dupliquer',
        ],
        'add-new-button' => 'Ajouter',
        'listing-empty-message' => 'Il n\'y a aucun contenu ici pour le moment.',
        'languages' => 'Langues',
    ],
    'main' => [
        'published' => 'Publié',
        'draft' => 'Brouillon',
        'create' => 'Créer',
        'update' => 'Mettre à jour',
        'title' => 'Titre',
    ],
    'modal' => [
        'title-field' => 'Titre',
        'permalink-field' => 'Permalien',
        'create' => [
            'title' => 'Ajouter',
            'button' => 'Créer',
            'create-another' => 'Créer et ajouter un(e) autre',
        ],
        'update' => [
            'title' => 'Mettre à jour',
        ],
    ],
    'nav' => [
        'media-library' => 'Media Library',
        'cms-users' => 'Utilisateurs du CMS',
        'settings' => 'Paramètres',
        'logout' => 'Déconnexion',
        'admin' => 'Admin',
    ],
    'media-library' => [
        'title' => 'Media Library',
        'images' => 'Images',
        'files' => 'Fichiers',
        'filter-select-label' => 'Filtrer par tag',
        'sidebar' => [
            'empty-text' => 'Pas de fichiers sélectionnés',
            'files-selected' => 'Fichiers sélectionnés',
            'clear' => 'Vider',
            'dimensions' => 'Dimensions',
        ],
    ],
    'filter' => [
        'search-placeholder' => 'Rechercher',
        'toggle-label' => 'Filtrer',
        'apply-btn' => 'Appliquer',
        'clear-btn' => 'Vider',
    ],
    'select' => [
        'empty-text' => 'Désolé, pas d\'options correspondantes trouvées.',
    ],
    'uploader' => [
        'dropzone-text' => 'ou faites glisser de nouveaux fichiers ici',
        'upload-btn-label' => 'Ajouter',
    ],
    'fields' => [
        'medias' => [
            'btn-label' => 'Joindre une image',
        ],
        'block-editor' => [
            'collapse-all' => 'Tout masquer',
            'expend-all' => 'Tout afficher',
            'open-in-editor' => 'Ouvrir dans l\'éditeur',
            'create-another' => 'Créer un(e) autre',
            'delete' => 'Supprimer',
            'add-content' => 'Ajouter du contenu',
            'preview' => 'Prévisualiser',
            'loading' => 'Chargement',
        ],
        'browser' => [
            'attach' => 'Joindre',
            'add-label' => 'Ajouter',
        ],
        'files' => [
            'add-label' => 'Ajouter',
        ],
    ],
    'user-management' => [
        'users' => 'Utilisateurs',
        'active' => 'Actif',
        'disabled' => 'Désactivé',
        'enabled' => 'Activé',
        'trash' => 'Corbeille',
        'user-image' => 'Image',
        'name' => 'Nom',
        'email' => 'Email',
        'role' => 'Rôle',
        'content-fieldset-label' => 'Paramètre utilisateur',
        'edit-modal-title' => 'Éditer le nom d\'utilisateur',
        'update-disabled-user' => 'Mettre à jour l\'utilisateur désactivé',
        'update-disabled-and-close' => 'Mettre à jour l\'utilisateur désactivé et fermer',
        'update-disabled-user-and-create-new' => 'Mettre à jour l\'utilisateur désactivé et créer un nouveau',
        'enable-user' => 'Activer l\'utilisateur',
        'enable-user-and-close' => 'Activer l\'utilisateur et fermer',
        'enable-user-and-create-new' => 'Activer l\'utilisateur et créer un nouveau',
        'update' => 'Mettre à jour',
        'update-and-close' => 'Mettre à jour et fermer',
        'update-and-create-new' => 'Mettre à jour et créer un nouveau',
        'cancel' => 'Annuler',
    ],
    'publisher' => [
        'switcher-title' => 'Status',
        'save' => 'Sauvegarder comme brouillon',
        'save-close' => 'Sauvegarder comme brouillon et fermer',
        'save-new' => 'Sauvegarder comme brouillon et crée un nouveau',
        'publish' => 'Publier',
        'publish-close' => 'Publier et fermer',
        'publish-new' => 'Publier et créer un nouveau',
        'update' => 'Mettre à jour',
        'update-close' => 'Mettre à jour et fermer',
        'update-new' => 'Mettre à jour et créer un nouveau',
        'cancel' => 'Annuler',
    ],
];
