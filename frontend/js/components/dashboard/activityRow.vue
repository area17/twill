      ...
      <template v-else>
        <template v-if="col.name === 'name'">
          ...
          <p class="activityCell__meta f--note">
            {{ row['activity'] }} <timeago :auto-update="1" :datetime="new Date(row['date'])"></timeago> {{ $trans('dashboard.activity-row.by') }} {{ row['author'] }}
    ...
    <td class="activityCell activityCell--icon">
      <a17-dropdown ref="activityRowSetupDropdown" position="bottom-right">
        ...
        <div slot="dropdown__content">
          <a v-if="row.hasOwnProperty('permalink')" :href="row['permalink']" target="_blank">{{ $trans('dashboard.activity-row.view-permalink') }}</a>
          <a v-if="row.hasOwnProperty('edit')" :href="row['edit']">{{ $trans('dashboard.activity-row.edit') }}</a>
