<template>
  <div class="datatable"  data-sticky-id="thead" data-sticky-offset="0">
    <!--Disable stickiness-->
    ...
    watch: {
      filterData: {
        handler: function (newVal) {
          ...
          if (!newVal || Object.keys(newVal).length === 0 || (Object.keys(newVal).length === 1 && newVal.search === '')) {
            this.$store.commit(DATATABLE.CLEAR_DATATABLE_FILTER)
          } else {
            this.$store.commit(DATATABLE.UPDATE_DATATABLE_FILTER, newVal)
          }
      ...
      searchData: {
        handler: function (newVal) {
          const currentFilter = { ...this.$store.state.mediaLibrary.filterData }
          currentFilter.search = newVal.search || ''
          this.$store.commit(DATATABLE.UPDATE_DATATABLE_PAGE, 1)
          this.$store.commit(DATATABLE.UPDATE_DATATABLE_FILTER, currentFilter)
          // reload datas
          this.$store.dispatch(ACTIONS.GET_DATATABLE)
        },
        deep: true
