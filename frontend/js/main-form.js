...
if (typeof window[import.meta.env.VITE_APP_NAME].TWILL_BLOCKS_COMPONENTS !== 'undefined') {
    window[import.meta.env.VITE_APP_NAME].TWILL_BLOCKS_COMPONENTS.map(componentName => {
...
const extractComponentNameFromContextKey = (contextKey) => `a17-${contextKey.match(/\w+/)[0].replace(
    /([a-z])([A-Z])/g, '$1-$2').replace(/\s+/g, '-').toLowerCase()}`

// const importedCustomBlocks = import.meta.glob('@/components/blocks/customs/*.{js|vue}')
...
// const importedTwillBlocks = import.meta.glob('@/components/blocks/*.{js|vue}')
//
...
// const importedComponents = import.meta.glob('@/components/customs/*.{js|vue}')
...
// const importedVendorComponents = import.meta.glob('@/components/customs-vendor/*.{js|vue}')
