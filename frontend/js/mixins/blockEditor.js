import cloneDeep from 'lodash/cloneDeep'

export default {
  props: {
    editorName: {
      type: String,
      required: true
    },
    blocks: {
      type: Array,
      default: () => []
    },
    savedBlocksLength: {
      type: Number,
      default: 0
    }
  },
  methods: {
    addAndEditBlock (add, edit, { block, index }) {
      window[import.meta.env.VITE_APP_NAME].PREVSTATE = cloneDeep(this.$store.state)
      add(block, index)
      edit(index)
    },
    saveBlock (fn, index) {
      if (window[import.meta.env.VITE_APP_NAME].hasOwnProperty('PREVSTATE')) delete window[import.meta.env.VITE_APP_NAME].PREVSTATE
      this.unselectBlock(fn, index)
      this.$emit('block:save', index)
    },
    cancelBlock (fn, index) {
      if (window[import.meta.env.VITE_APP_NAME].hasOwnProperty('PREVSTATE')) {
        // eslint-disable-next-line
        console.warn('Store - Restore previous Store state')
        this.$store.replaceState(window[import.meta.env.VITE_APP_NAME].PREVSTATE)
      }
      this.unselectBlock(fn, index)
      this.$emit('block:cancel', index)
    },
    selectBlock (fn, index) {
      window[import.meta.env.VITE_APP_NAME].PREVSTATE = cloneDeep(this.$store.state)
      fn()
      this.$emit('block:select', index)
    },
    unselectBlock (fn, index) {
      fn()
      if (window[import.meta.env.VITE_APP_NAME].hasOwnProperty('PREVSTATE')) delete window[import.meta.env.VITE_APP_NAME].PREVSTATE
      this.$emit('block:unselect', index)
    },
    moveBlock (index) {
      this.$emit('block:move', index)
    },
    deleteBlock (fn) {
      if (this.$root.$refs.warningContentEditor) {
        this.$root.$refs.warningContentEditor.open(() => {
          fn()
          this.$emit('block:delete')
        })
      } else {
        fn()
        this.$emit('block:delete')
      }
    }
  }
}
