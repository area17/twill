<nav
    class="sidebar w-3-cols xl:w-240 bg-primary sticky top-[80px] h-screen-minus-header pr-32 pb-16 border-r border-primary overflow-x-hidden overflow-y-auto"
                    ...
                    @foreach ($tree[$currentSegment]['items'] ?? [] as $item)
                        ...
                        <li
                            ...
                            <div class="flex items-center">
                                <a class="inline no-underline hover:text-tip
                                    @if ($open) text-tip font-medium no-underline @endif"
                                    href="{{ $item['url'] ?? '#' }}">
                                        {{ $item['title'] ?? '' }}
                                </a>
                                ...
                                <ul class="overflow-hidden pl-[12px] {{!$open ? 'max-h-0' : ''}}"
                                    ...
                                    @foreach ($item['items'] ?? [] as $item)
                                        ...
                                        <li class="relative pt-8">
                                            <a
                                              class="block w-full pl-3.5 before:pointer-events-none text-primary no-underline hover:text-tip @if($active) font-medium text-tip @endif"
                                              href="{{ $item['url'] ?? '#' }}">
                                              {{ $item['title'] ?? '' }}
                                            </a>
                                        </li>
