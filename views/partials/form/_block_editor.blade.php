@php
    ...
    $used = array_values(array_filter($form_fields['blocks'] ?? [], function ($block) use ($name) {
...
@push('vuexStore')
    ...
    window['{{ config('twill.js_namespace') }}'].STORE.form.blocks.used['{{ $name }}'] = {!! json_encode($used) !!}
