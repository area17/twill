...
@section('form')
    ...
    @foreach(config('twill.oauth.enabled', []) as $index => $provider)
        <a href="{!! route('admin.login.redirect', $provider) !!}" class="login__google login__{{$provider}}" tabindex="{{ 4 + $index }}">

            {{-- TODO: Implement icons for all social networks --}}
