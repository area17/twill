...
<div class="form" v-sticky data-sticky-id="navbar" data-sticky-offset="0" data-sticky-topoffset="12">
    <div class="navbar navbar--sticky" data-sticky-top="navbar">
        ...
        <a17-sticky-nav data-sticky-target="navbar" :items="{{ json_encode($additionalFieldsets) }}">
            <a17-title-editor name="{{ $titleFormKey }}" thumbnail="{{ $titleThumbnail ?? '' }}" :editable-title="{{ json_encode($editableTitle ?? true) }}" :control-languages-publication="{{ json_encode($controlLanguagesPublication) }}" custom-title="{{ $customTitle ?? '' }}" custom-permalink="{{ $customPermalink ?? '' }}" localized-permalinkbase="{{ json_encode($localizedPermalinkBase ?? '') }}" localized-custom-permalink="{{ json_encode($localizedCustomPermalink ?? '') }}" slot="title" @if($createWithoutModal ?? false) :show-modal="true" @endif @if(isset($editModalTitle)) modal-title="{{ $editModalTitle }}" @endif>
            ...
            <div class="wrapper wrapper--reverse" v-sticky data-sticky-id="publisher" data-sticky-offset="80">
                <aside class="col col--aside">
                    <div class="publisher" data-sticky-target="publisher">
                        <a17-publisher {!! !empty($publishDateDisplayFormat) ? "date-display-format='{$publishDateDisplayFormat}'" : '' !!} {!! !empty($publishDateFormat) ? "date-format='{$publishDateFormat}'" : '' !!} {!! !empty($publishDate24Hr) && $publishDate24Hr ? ':date_24h="true"' : '' !!} :show-languages="{{ json_encode($controlLanguagesPublication) }}">
                            @yield('publisherRows')
                        </a17-publisher>
...
<script>
    const groupUserMapping = {
        !!isset($groupUserMapping) ? json_encode($groupUserMapping) : '[]'!!
    };
    window['{{ config('
        twill.js_namespace ') }}'].vm.$store.subscribe((mutation, state) => {
            ...
            if (!isNaN(groupId)) {
                ...
                users.forEach(function(userId) {
                    ...
                    if (currentPermission === '' || currentPermission === 'view-item') {
                        ...
                        window['{{ config('
                            twill.js_namespace ') }}'].vm.$store.commit('updateFormField', field)
                    }
                })
            }
        }
