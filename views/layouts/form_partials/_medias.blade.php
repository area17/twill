...
<script>
    var media_library_options_{{ $media_role }} = {
      "role": "{{ $media_role }}",
...
<section id="media_library" class="box {{ $media_class }} box-background" data-behavior="media_library" data-options="media_library_options_{{ $media_role }}">
    ...
    <div data-media-bucket="{{ $media_role }}" data-media-template="{{ moduleRoute($moduleName, $routePrefix, 'media', ['with_crop' => $with_crop, 'with_multiple' => $with_multiple, 'with_background_position' => $with_background_position]) }}" data-media-item=".media-row" data-behavior="{{ $with_multiple ? 'sortable_box' : ''}}">
